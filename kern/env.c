...
// curenv Seems to be unecessary for lab4.
...
int envid2env(envid_t envid, struct Env **env_store, bool checkperm)
{
	...
	e = &envs[ENVX(envid)];
	// cprintf("	e->env_status = %d, e->env_id = %x, envid = %x\n", e->env_status, e->env_id, envid);
...
load_icode(struct Env *e, uint8_t *binary)
{
	...
	for (; ph < eph; ph++)
	{
		...
		if (ph->p_va + ph->p_memsz > e->env_heap_marker)
			e->env_heap_marker = ph->p_va + ph->p_memsz;
	}
