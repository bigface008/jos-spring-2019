...
serve_read(envid_t envid, union Fsipc *ipc)
{
	...
	ssize_t num;
	if ((num = file_read(open_file->o_file, ret->ret_buf, req->req_n, open_file->o_fd->fd_offset)) < 0)
		return num;
	open_file->o_fd->fd_offset += num;
	return (int)num;
}
...
serve_write(envid_t envid, struct Fsreq_write *req)
{
	...
	int num;
	if ((num = file_write(open_file->o_file, req->req_buf, req->req_n, open_file->o_fd->fd_offset)) < 0)
		return num;
	open_file->o_fd->fd_offset += num;
	return num;
}
